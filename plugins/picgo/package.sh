#!/bin/bash
# Package the picgo plugin into a zip that can be installed in SwallowNote.
#
# The zip file is named after the plugin id (com.swallownote.picgo.zip)
# and contains:
#   - index.js          (ES module bundle built by Vite)
#   - manifest.json     (plugin metadata)
#   - settings.json     (settings schema; host reads this on install
...
PLUGIN_ID="com.swallownote.picgo"
...
# baked into the bundled `index.js` (the bundler inlines manifest.json
# via `import manifestJson from '../manifest.json'` and Vite bundles
# the JSON, so the new version string flows into the zip through
# the Vite output).
...
PY
  ...
  # src/index.tsx — this plugin's entry imports the manifest from
  # `../manifest.json` and spreads it (`...manifestJson`), so the
  # `version` literal does NOT live in the TS source. The Vite
  # bundler inlines the JSON module at build time, so the new
  # value propagates automatically. We skip the regex update when
  # the file has no version literal to avoid a misleading warning.
  if grep -Eq "version:\s*['\"]${OLD_VERSION}['\"]" "$SRC_INDEX"; then
...
PY
    ...
    echo "  → src/index.tsx has no version literal (uses ...manifestJson spread) — skipping"
  fi
...
# runs produce zips with different SHA-256 even though the
# *contents* are byte-identical — the host's plugin installer
# compares the on-the-wire zip hash against the value declared in
# `plugins/repo.json`, and a non-deterministic build means we have
# to bump `repo.json`'s `sha256` on every rebuild, even when no
# code changed.
#
# We touch the directory entries too (`dist/`) because `zip`
# records the directory's mtime in the central directory and that
# field is *not* stripped by `-X` — leaving it alone would still
# leak the current system clock into the final hash.
touch -d '2020-01-01T00:00:00Z' \
  "$DIST_DIR/index.js" \
  "$DIST_DIR/manifest.json" \
  "$DIST_DIR/settings.json" \
  "$DIST_DIR"
zip -X -r "$SCRIPT_DIR/$ZIP_NAME" index.js manifest.json settings.json > /dev/null
