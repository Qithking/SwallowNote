...
# Make sure the manifest that ships inside the zip matches the
# post-bump value we just wrote. `vite build` doesn't touch
# `dist/manifest.json` directly — we copy it from the source so the
# published artifact is internally consistent.
