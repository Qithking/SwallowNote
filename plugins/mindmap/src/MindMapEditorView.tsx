...
/**
 * `MindMapEditorView` is the React component the host mounts for
 * `.smm` files. It is registered via the manifest's
 * `editorComponent` field and the host passes the current note
 * content + an `onChange` callback that the component uses to
 * persist edits.
 *
 * The implementation is a 1:1 port of the host's
 * `src/components/editors/MindMapEditor.tsx`. Adaptations:
 *
 *   1. `useUIStore` is replaced with `usePluginTheme`, which
 *      follows the OS `prefers-color-scheme` media query.
 *   2. `useTranslation` is replaced with the plugin-local
 *      `useT` hook (see `./i18n/useT.ts`).
 *   3. UI primitives are imported from the plugin's local
 *      `./components/ui/*` instead of `@/components/ui/*`.
 *
 * The host's editor dispatcher only passes `{ content, onChange }`,
 * so this component intentionally does NOT take a `PluginPanelProps`.
 * Live locale/theme updates from the host would require wiring up
 * the editor's subscription through a lifecycle hook; for the
 * initial migration the editor follows the OS theme + a static
 * zh-CN locale (overridable in standalone previews).
 */
interface MindMapEditorViewProps {
  content: string
...
export function MindMapEditorView({ content, onChange }: MindMapEditorViewProps) {
  const t = useT()
  ...
  // `usePluginTheme` already resolves system via the media-query
  // listener. We just keep a ref so the init effect (which runs
  // before React's commit) can read the latest value.
